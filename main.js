window.addEventListener("DOMContentLoaded", function (event) {
  ...
  window.addEventListener("keydown", function (event) {
    ...
    if (event.key == "H" || event.key == "h") {
      ...
      noteElement.innerHTML = `여러운 모드입니다. 스페이스키를 눌러 시작하세요!`;
    ...
    if (event.key == "E" || event.key == "e") {
      ...
      noteElement.innerHTML = `쉬운 모드입니다. 스페이스키를 눌러 시작하세요!`;
      noteElement.style.opacity = 1;
      resetGame();
      return;
    }
  ...
  function main(timestamp) {
    try {
      ...
      const pressSpaceToStart = "스페이스 키를 눌러 게임을 재시작하세요.";
      const changeMode = hardMode
        ? "쉬운 모드로 돌아갈까요? 문자 E를 누르세요."
        : "어려운 모드에 준비가 되었나요? 문자 H를 누르세요.";
      const followMe =
        '노션링크 <a href="https://atentsgame.com" , target="_blank">아텐츠게임아카데미</a>';
  ...
  function getNextPosition() {
    ...
    switch (snakeDirection) {
      ...
      case "up": {
        const nextPosition = headPosition - width;
        if (nextPosition < 0) throw Error("뱀이 벽에 막혔습니다");
        // Ignore the last snake part, it'll move out as the head moves in
        if (snakePositions.slice(1).includes(nextPosition))
          throw Error("뱀이 스스로를 물었습니다");
        return nextPosition;
  ...
  function getDirection(first, second) {
    ...
    throw Error("두 타일이 연결되어 있지 않습니다");
  }
